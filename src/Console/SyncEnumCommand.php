...
METHOD;
    ...
    protected function extractCanTransitionToMethod(string $enumPath): ?string
    {
        ...
        /** @var array{0: non-falsy-string, 1: int<0, max>} $matchInfo */
        $matchInfo = $matches[0];
        $signatureStart = $matchInfo[1];
        $signatureEnd = $signatureStart + strlen($matchInfo[0]);
        ...
        if (preg_match_all('/\/\*\*[\s\S]*?\*\//', $beforeSignature, $docMatches, PREG_OFFSET_CAPTURE)) {
            // Get the last match
            /** @var array{0: string, 1: int} $lastDocblock */
