...
describe('CompositeChecker::all() - AND Logic', function (): void {
    ...
    /**
     * Scenario: Require both role-based AND policy-based authorization (strict security)
     * Setup: Admin user with role check passing AND gate check passing
     * Assertions: all() returns true only when both checkers approve the transition
     */
    ...
    /**
     * Scenario: AND logic requires all checks to pass; role check failure denies despite gate approval
     * Setup: Guest user (fails role check) with permissive gate (passes policy check)
     * Assertions: Denied because RoleBasedChecker rejects, even though PolicyBasedChecker allows
     */
    ...
    /**
     * Scenario: AND logic fails if any checker denies; policy denial blocks despite valid role
     * Setup: Admin user (passes role check) with restrictive gate (fails policy check)
     * Assertions: Denied because PolicyBasedChecker rejects, overriding role permission
     */
    ...
    /**
     * Scenario: AND logic with both checkers denying is definitive rejection
     * Setup: Guest user (no role) with restrictive gate (policy denies)
     * Assertions: Denied by both RoleBasedChecker and PolicyBasedChecker
     */
