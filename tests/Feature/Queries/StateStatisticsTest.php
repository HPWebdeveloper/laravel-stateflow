...
describe('StateStatistics - Count By State', function () {
    ...
    /**
     * Scenario: Get distribution counts for all states (dashboard metrics)
     * Setup: Create posts in various states
     * Assertions: Returns dict with state=>count (draft:2, review:1, published:1)
     */
    ...
    /**
     * Scenario: Empty table returns empty collection (graceful no-data handling)
     * Setup: Truncate posts table
     * Assertions: Returns empty collection not error (safe for new systems)
     */
...
describe('StateStatistics - Percentage By State', function () {
    ...
    /**
     * Scenario: Calculate percentage distribution across states (analytics/reporting)
     * Setup: 4 posts: 2 draft (50%), 1 review (25%), 1 published (25%)
     * Assertions: Returns state=>percentage dict totaling 100% for pie charts
     */
    ...
    /**
     * Scenario: Empty table returns empty for percentages (avoid division by zero)
     * Setup: Truncate posts table
     * Assertions: Returns empty collection not NaN or error (safe math)
     */
    it('returns empty collection for empty table', function () {
        Post::truncate();
...
describe('StateStatistics - Transition Patterns', function () {
    ...
    /**
     * Scenario: Identify most frequent transition paths (workflow optimization insights)
     * Setup: Record 3x draft->review, 1x review->published transitions
     * Assertions: Returns ordered list with draft->review first (count=3, most common)
     */
    ...
    /**
     * Scenario: Limit top N transitions for dashboard display (pagination)
     * Setup: Query mostCommonTransitions with limit: 1
     * Assertions: Returns only top result (performance optimization for summaries)
     */
    ...
    /**
     * Scenario: No recorded transitions returns empty (new system or no history enabled)
     * Setup: Query statistics without recording any transitions
     * Assertions: Returns empty collection not error (graceful degradation)
     */
